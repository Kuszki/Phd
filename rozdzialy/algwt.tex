...
Ze względu możliwość analizy sygnału jednocześnie w dziedzinie częstotliwości oraz w dziedzinie czasu, algorytmy transformacji falkowej są wykorzystywane w wielu dziedzinach~\cite{akujuobi_applications, ahmad_wavelet, wallen_handbook}, przy czym między innymi stosuje się je:
\begin{itemize}
\item w medycynie, do analizy sygnału EEG oraz EKG~\cite{ocak_medicine, unser_medicine},
\item w przetwarzaniu obrazu oraz dźwięku~\cite{kotteri_imagecomp, averbuch_spline},
\item w algorytmach kompresji danych~\cite{reddy_compression},
\item w analizie drgań sejsmicznych~\cite{anping_seismic},
...
\item w energetyce, podczas analizy zwarć wysokoprądowych~\cite{niedopytalski_zwar},
\item w procesie redukcji szumu w sygnale pomiarowym~\cite{auth_denoise, vimala_ddendenoise}.
\end{itemize}
W zależności od właściwości analizowanego sygnału, stosuje się odpowiednią w danym przypadku falkę-matkę, której właściwości pozwalają na detekcję analizowanego zjawiska. Ze względu na fakt, że algorytmy te stanowią istotną część toru pomiarowego, ich właściwości metrologiczne nie mogą być pomijane podczas analizy właściwości metrologicznych tego obiektu, co zaznaczono we wstępie do pracy.
