...
Podstawiając falkę opisaną równaniem \eqref{eqn_dwt_wavelet} do równania \eqref{eqn_cwt} otrzymuje się zależność opisującą dyskretną transformację falkową:
\begin{equation}
w_{m,n} = \frac{1}{\sqrt{2^{m}}} \int _{-\infty} ^{\infty} x \left( t \right) \psi \left( \frac{t-2n^{m}}{2^{m}} \right) dt \label{eqn_dwt}.
\end{equation}
...
Stosując podobne założenia, jak w równaniu \eqref{eqn_dwt_wavelet}, otrzymuje się równanie funkcji skalującej dla dyskretnej wartości numeru skali i przesunięcia w czasie:
\begin{equation}
\phi_{m,n} \left( t \right) = \frac{1}{\sqrt{2^{m}}} \phi \left( \frac{t-2n^{m}}{2^{m}} \right) dt \label{eqn_dwt_scalefun},
\end{equation}
...
\begin{equation}
\int _{-\infty} ^{\infty} \phi_{0,0} \left( t \right) dt = 1 \label{eqn_dwt_fatherwav},
\end{equation}
...
\label{eqn_db2_outvect_t_1_0_full}, \\
T_{1,1} =
...
\label{eqn_db2_outvect_t_1_1_full}, \\
T_{1,2} =
...
\label{eqn_db2_outvect_t_1_2_full}, \\
T_{1,3} =
...
\label{eqn_db2_outvect_t_1_3_full}.
\end{gather}
