...
Jako, że wprowadzenie okna pomiarowego objawia się zmianą wypadkowej transmitancji związanej z wielkościami wyjściowymi algorytmu, działanie to wpływa na przenoszone z wejścia na wyjście algorytmu sygnały błędów. Przedstawiony w pracy model błędu algorytmu uwzględnia możliwość analizy właściwości metrologicznych tego algorytmu w omawianym przypadku i wymaga jedynie wyznaczenia zgodnie z treścią równania~\eqref{eq:alg_trans_single} transmitancji algorytmu dla współczynników macierzy transformacji wyznaczonych zgodnie z zależnością~\eqref{eq:wt_windowmod}. Należy zaznaczyć, że w omawianym przypadku transmitancja dla każdej wielkości wyjściowej algorytmu może być inna. Aplikację omawianej metody oraz wpływ parametrów okna pomiarowego na przenoszenie błędów losowych przez algorytmy dyskretnej transformacji falkowej opisano w pracy~\cite{auth_window}.
