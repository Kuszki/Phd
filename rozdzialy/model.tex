...
Sygnały błędów obecne w sygnale wyjściowym dowolnego obiektu podzielić można, zgodnie z wcześniejszymi rozważaniami, na statyczne, dynamiczne i losowe. Podział ten natomiast należy rozszerzyć pod kątem genezy sygnału błędu, przy czym wyróżnia się w tym przypadku błędy własne oraz propagowane. Jako błędy własne rozumie się te, które analizowany obiekt wprowadza do sygnału wyjściowego, a za ich genezę rozumie się niedoskonałe właściwości tego obiektu. Błędy propagowane stanowią natomiast sygnały błędów zawarte w sygnale wejściowym, przetworzone przez obiekt zgodnie z jego rzeczywistą charakterystyką, a następnie dodane do sygnału wyjściowego tego obiektu. Suma sygnałów błędów własnych i propagowanych dla danej grupy błędów stanowi wypadkowy sygnał błędu tej grupy.
...
\end{gather}
gdzie wypadkowy sygnał błędu $e_{u,\Sigma}(t)$ składać się będą przetwarzane i wprowadzane przez analizowany fragment sygnały błędów statycznych, dynamicznych oraz losowych.
...
\end{equation}
gdzie $e_{x,s}(i)$ jest błędem statycznym, $e_{x,d}(i)$ błędem dynamicznym, natomiast $e_{x,r}(i)$ błędem losowym wielkości wejściowej obiektu oraz:
\begin{gather}
...
\end{gather}
Błąd wypadkowy $e_{x,\Sigma}(i)$ zawarty w przetwarzanym sygnale można zatem wyrazić w postaci sumy wszystkich wymienionych w równaniu~\eqref{eq:in_disc_real} błędów cząstkowych jako:
\begin{equation}
e_{x,\Sigma} \emb{i} = e_{x,s} \emb{i} + e_{x,d} \emb{i} + e_{x,r} \emb{i} \label{eq:in_disc_err_sum}.
\end{equation}
Przedstawiony sygnał błędu przetwarzany jest przez analizowany obiekt i propagowany na jego wyjście, zgodnie z charakterystyką tego obiektu.

Podobnie jak w przypadku opisu części analogowej, rozważyć należy wpływ transmitancji $H_{X}(z)$ na przetwarzany sygnał $x(i)$, przy czym sygnał wyjściowy fragmentu obiektu związanego z omawianą transmitancją oznaczono symbolem $v(i)$. Biorąc pod uwagę wcześniejsze założenie, gdzie transmitancja $G_{X}(j\omega)$ odpowiada transmitancji $H_{X}(z)$ dla zastosowanego okresu próbkowania, parametry związane z tą transmitancją, stanowiące odpowiednik równań~\eqref{eq:mid_cont_amp} oraz~\eqref{eq:mid_cont_phi}, opisać można jako:
\begin{gather}
...
\end{gather}
gdzie $K_{u}(\omega)$ stanowi wzmocnienie oraz $\varphi_{u}(\omega)$ przesunięcie w fazie wprowadzane przez obiekt. Związek pomiędzy przedstawionymi postaciami transmitancji obiektu w dziedzinie $\mathcal{Z}$ oraz $\mathcal{F}$ wyjaśniono w dalszej części pracy. Wobec przedstawionych założeń równanie~\eqref{eq:mid_cont_var_omega} przyjmuje postać:
\begin{equation}
\sigma_{v}^{2} \emb{\omega} = K_{X}^{2} \emb{\omega} \sigma_{x}^{2} \emb{\omega} \label{eq:mid_disc_var_omega},
\end{equation}
gdzie $\sigma_{x}^{2}(\omega)$ jest wariancją sygnału na wejściu, natomiast $\sigma_{X}^{2}(\omega)$ wariancją sygnału na wyjściu fragmentu reprezentującego właściwości dynamiczne obiektu.
...
\end{gather}
przy czym $e_{v,\Sigma}(i)$ jest wypadkowym sygnałem błędu, stanowiącym sumę wszystkich sygnałów błędów wielkości wyjściowej fragmentu związanego z właściwościami dynamicznymi analizowanego obiektu.

W przypadku błędów statycznych, opisanych dla części analogowej równaniami~\eqref{eq:mid_cont_err_stat_self} oraz~\eqref{eq:mid_cont_err_stat_prop}, w analizowanym przypadku zapisać można:
\begin{gather}
e_{v,sw} \emb{i} = \left( \tilde{K}_{X} \emb{0} - \dot{K}_{X} \emb{0} \right) E_{x,o} \emb{0} \label{eq:mid_disc_err_stat_self}, \\
e_{v,sp} \emb{i} = \tilde{K}_{X} \emb{0} E_{x,e} \emb{0} \emb{0} \label{eq:mid_disc_err_stat_prop},
\end{gather}
gdzie $e_{v,sw}(t)$ jest błędem statycznym własnym, natomiast $e_{v,sp}(t)$ jest błędem statycznym propagowanym przez fragment obiektu związany z jego właściwościami dynamicznymi.
