...
Zaproponowane na rysunkach~\ref{fig:chain_cont} oraz~\ref{fig:chain_disc} modele bazują na koncepcji modelu Wienera, opisanej szerzej między innymi w publikacji~\cite{wills_nonlinear}. Jako, że rozważania zawarte w pracy ograniczone zostały do przypadku zakładającego liniowy charakter funkcji przetwarzania, zastosowanie zaproponowanego modelu umożliwia podjęcie dalszych badań obejmujących przypadki adekwatne dla obiektów nieliniowych. Dodatkowo zauważyć można, że przyjmując założenia odnośnie liniowości obiektów, przedstawiona w pracy metoda analizy jest identyczna stosując model Hammersteina~\cite{wills_nonlinear}.
