...
\begin{table}[htb!]
\begin{center}
\makecaption{tab:comp_reductive}{Zestawienie uzyskanych metodą Monte-Carlo przedziałów wartości realizacji względnego błędu oszacowania wypadkowej wartości niepewności rozszerzonej obejmujących~\qty{95}{\percent} wartości realizacji tej wielkości w zależności od stosowania zaproponowanej w pracy korekty współczynnika kształtu (wartości wyników wyrażone w procentach)}
...
\makecaption{fig:hist_reductive}{Histogramy wartości realizacji względnego błędu oszacowania wypadkowej wartości niepewności rozszerzonej dla zaproponowanej w pracy metody odpowiadające danym zestawionym w tabeli~\ref{tab:comp_reductive} dla przypadku stosowania zaproponowanej w pracy korekty}
\end{center}
\end{figure}
