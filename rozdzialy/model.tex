...
\end{equation}
gdzie $\sigma_{x}^{2}(\omega)$ jest wariancją sygnału na wejściu, natomiast $\sigma_{v}^{2}(\omega)$ wariancją sygnału na wyjściu fragmentu reprezentującego właściwości dynamiczne obiektu, oznaczonego symbolem $v(i)$. W celu wyznaczenia transmitancji algorytmu w dziedzinie pulsacji należy dla $H_{X}(z)$ przyjąć założenie $z = e^{2\pi j \frac{\omega}{\omega_{p}}}$, gdzie $\omega_{p}$ jest pulsacją próbkowania.
...
\end{equation}
W celu wyznaczenia transmitancji algorytmu w dziedzinie pulsacji należy przyjąć w równaniu~\eqref{eq:alg_trans_single} założenie $z = e^{2\pi j \frac{\omega}{\omega_{p}}}$, gdzie $\omega_{p}$ jest pulsacją próbkowania.
