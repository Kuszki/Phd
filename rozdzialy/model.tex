...
Należy zauważyć, że rzeczywisty przetwornik analogowo-cyfrowy wprowadzać będzie do sygnału wyjściowego dodatkowe błędy związane między innymi z nieliniowością charakterystyki przetwarzania, przesunięciem tej charakterystyki, niedoskonałością źródła napięcia referencyjnego, impedancją wejściową układu próbkująco-pamiętającego, niejednorodnością właściwości wzorca, czy wpływem czynników zewnętrznych. Dodatkowo sygnały błędów na wyjściu takiego przetwornika mogą być skorelowane z wielkością wejściową oraz sygnałem szumu zawartym w tej wielkości, przy czym zwykle korelacja ta jest pomijalnie mała~\cite{sienkowski_adc}. Analizę właściwości wybranych przetworników cyfrowo-analogowych przedstawiają szczegółowo między innymi prace~\cite{jakubiec_przetwarzanie, jakubiec_system, sienkowski_adc, sienkowski_kwant, arpaia_deltasigma}. Należy zatem określić budżet niepewności dla zastosowanego w torze pomiarowym przetwornika, a następnie na jego podstawie zdefiniować dokładną postać składników równań~\eqref{eq:adc_outerr} oraz~\eqref{eq:adc_rerr}.
...
Zaproponowane na rysunkach~\ref{fig:chain_cont} oraz~\ref{fig:chain_disc} modele bazują na koncepcji modelu Wienera, opisanej szerzej między innymi w publikacji~\cite{wills_nonlinear}. Jako, że rozważania zawarte w pracy ograniczone zostały do przypadku zakładającego liniowy charakter funkcji przetwarzania, zastosowanie zaproponowanego modelu umożliwia podjęcie dalszych badań obejmujących przypadki adekwatne dla obiektów nieliniowych. Zauważyć można, że przyjmując założenia odnośnie liniowości obiektów, przedstawiona w pracy metoda analizy jest identyczna stosując model Hammersteina~\cite{wills_nonlinear}. Dla obiektów, których opis właściwości za pomocą modelu Hammersteina jest bardziej korzystny od zaproponowanego modelu Wienera, istnieje możliwość przedstawienia właściwości tych obiektów za pomocą dwóch połączonych kaskadowo elementów. Element pierwszy reprezentować będzie funkcję przetwarzania oraz niezależną od pulsacji jednostkową transmitancję. Elementy drugi reprezentować będzie funkcję przetwarzania o jednostkowym wzmocnieniu oraz rzeczywistą transmitancję obiektu.
