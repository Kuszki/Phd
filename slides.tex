...
\item Analiza właściwości metrologicznych jest najczęściej pomijana.
\end{itemize}
\end{frame}
...
\newsection{Niepewność rozszerzona}{Omówienie stosowanej metody wyznaczania wartości wypadkowej niepewności rozszerzonej}
...
\item Zaproponowany w pracy model błędów:
	\begin{itemize}
	\item symulacyjna weryfikacja przedstawionych zależności dla przykładowego toru pomiarowego,
...
\begin{figure}
\includegraphics[scale = 0.85]{obrazki/hist_rederr_all_1_10}
\caption{Względny błąd oszacowania wartości wypadkowej niepewności rozszerzonej \textbf{a)}~stosując proponowaną metodę, \textbf{b)}~stosując oryginalną metodę (bez korekty), \textbf{c)}~stosując przewodnik JCGM, $N \in \interval{3}{9}$, $U_{i} \in \interval{1}{10}$, $1-\alpha = \qty{95}{\percent}$}
\end{figure}
\end{frame}
...
\begin{table}
\caption{Zestawienie wyników eksperymentu symulacyjnego \textbf{a)}~stosując proponowaną metodę, \textbf{b)}~stosując oryginalną metodę (bez korekty), \textbf{c)}~stosując przewodnik JCGM, \textbf{s)}~stosując symulację metodą Monte Carlo (średnia dotyczy wszystkich wielkości)}
\begin{tabular}[c]{| c *{4}{|S[table-format = 2.2]} *{3}{|S[table-format = +1.2]} |} \hline
...
\multicolumn{5}{|c|}{Średnia wartości bezwzględnych wielkości $\delta_{*}$} & 5.77 & 1.48 & 6.13 \\ \hline
\end{tabular}
\end{table}
\end{frame}
