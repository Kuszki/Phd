function h = draw_brace(start, stop, width, varargin)
% draw_brace   Draw curly brace on current figure
%	draw_brace([X1, Y1], [X2, Y2]) draws a curly brace from the point [X1,
%	Y1] to the point [X2, Y2]
%
%	draw_brace([X1,Y1], [X2,Y2], W) draws a brace using the brace width W.
%
%	draw_brace(..., 'Param1', 'Value1', 'Param2', 'Value2') draws a brace
%	using the LineSeries property values specified by 'Param1'/'Value1',
%	'Param2'/'Value2', ... .
%
%	h = draw_brace(...) returns the LineSeries handle to the brace.
%
%	Example:
%	   h = draw_brace([0 0], [1 1], 20, 'Color', 'k')
